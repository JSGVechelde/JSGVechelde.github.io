function logicvariables() {
    ...
    */
    ...
    sessionStorage.setItem('mathematik_w', true)
    ...
    sessionStorage.setItem('mathematik_wl', true)
...
function logicsubjects(i) {
    switch (sessionStorage.getItem('pfach' + i)) {
        case 'Mathematik':
            ...
            kernfaecher_add()
            bereich_add('C')
            waehlbarkeit('mathematik_w', false)
            waehlbarkeit('mathematik_wl', false)
        ...
        case 'Chemie':
            ...
            if (i === 2) {
                switch (sessionStorage.getItem('pfach1')) {
                    case 'Mathematik': //<-- Das Brauchen wir nicht! nur ein Beispiel
                        break
                    case 'Physik':
                        waehlbarkeit('biologie_wl', false)
                        waehlbarkeit('geschichte_wl', false)
    ...
    let Anzahlen = [
            ...
            ['B_anzahl', 'C_anzahl', 'A_anzahl'],
            ['C_anzahl', 'A_anzahl', 'B_anzahl']
        ]
        //Die Wählbarkeit der Bereiche wird in diesem Array gespeichert.
    let waehlbarkeit_bereiche = [
        ['w[0]', 'w[1]', 'w[2]'],
        ['w[1]', 'w[2]', 'w[0]'],
        ['w[2]', 'w[0]', 'w[1]']
    ]
...
function proof_in_advance(pfach, i) {
    ...
    var all_subjects = Array('', 'Mathematik', 'Chemie', 'Englisch', 'Physik', 'Biologie', 'Franzoesisch', 'Latein', 'Musik', 'Informatik', 'Kunst', 'Spanisch', 'Deutsch', 'Geschichte', 'Politik-Wirtschaft', 'Religion', 'Erdkunde', 'Sport')
    for (let j = 0; j < 18; j++) {
        switch (all_subjects[j]) {
            case 'Mathematik':
                try {
                    if (i < 4) {
                        proof_in_advance_visibility(pfach, i, j, sessionStorage.getItem('w[2]'), sessionStorage.getItem('p2_waehlbar[0]'), sessionStorage.getItem('mathematik_wl'))
                    } else {
                        proof_in_advance_visibility(pfach, i, j, sessionStorage.getItem('w[2]'), sessionStorage.getItem('p2_waehlbar[0]'), sessionStorage.getItem('mathematik_w'))
                    }
                } catch {}
                break;
            case 'Chemie':
...
*/
//Methode, welche grundlegende Dinge für die Grundkurse im mathematikmatisch-naturwissenschaftlichen Profil regelt
function mana() {
    for (let j = 1; j < 6; j++) {
...
function pflicht_label() {
    ...
    if (deutsch_help == true && mathematik_help == true && geschichte_help == true && powi_help == true) {
        text += ' --- / --- '
    }
    document.getElementById("pflicht").textContent = text
}
...
function pflicht(j) {
    ...
    if (sessionStorage.getItem('extra12') == 'false' && sessionStorage.getItem('pfach' + j) == 'Mathematik') {
        sessionStorage.setItem('extra12', false)
    } else {
